...
%  ff=fffl+(ffcl-fffl)*iifrac;
  ...
  ccp2=[ccp(1);2*ccp(2:fftl/2);zeros(fftl/2,1)];
...
while (idx < nsyn-fftl) & (ii<length(f0l));
  ...
  if f0raw(ii) == 0; %mixNhigh(ii)>0.03
    ...
    ccp2=[ccp(1);2*ccp(2:fftl/2);zeros(fftl/2,1)];%covert double size spec to single
    %lft is simmilar to LPF,remove signal upon pi/2
    ffx=(fft(ccp2.*lft)/fftl); 


	tx=fftshift(real(ifft(exp(ffx)))); %fftshift :swap left right part of array
    % DC component removal 
    rx=randn([round(nf0),1]);
    tnx=fftfilt(rx-mean(rx),tx);
                                     %FIR filter fftfilt(b,x) = y(n) = b(1)x(n)+b(2)x(n-1)+....
