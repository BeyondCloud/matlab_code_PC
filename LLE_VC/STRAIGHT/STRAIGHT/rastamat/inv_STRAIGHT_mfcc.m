function spec = inv_STRAIGHT_mfcc(cep, sr, varargin)
...
[wintime, hoptime, numcep, lifterexp, sumpower, preemph, dither, ...
 minfreq, maxfreq, nbands, bwidth, dcttype, fbtype, usecmp, modelorder] = ...
    process_options(varargin, 'wintime', 0.025, 'hoptime', 0.010, ...
          'numcep', 24, 'lifterexp', 0.6, 'sumpower', 1, 'preemph', 0.97, ...
	  'dither', 0, 'minfreq', 0, 'maxfreq', sr/2, ...
...
pspc = cep2spec(cep, nbands+2*usecmp, dcttype);

if (usecmp)
  aspc = invpostaud(pspc, maxfreq, fbtype);
else
  aspc = pspc;
end

% Undo the auditory spectrum (inline)
spec = invaudspec(aspc, sr, 1024, fbtype, minfreq, maxfreq, sumpower, bwidth);

% % Back to waveform (modulate white noise)
% x = invpowspec(spec, sr, wintime, hoptime);
% 
% if preemph ~= 0
