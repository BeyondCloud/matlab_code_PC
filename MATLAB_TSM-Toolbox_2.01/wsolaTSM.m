...
yWinPos = 1:synHop:outputLength + ... % Positions of the synthesis
    winLenHalf;                         % windows in the output
xWinPos = ...                         % Positions of the analysis windows
    round(interp1(anchorPoints(:,2),... % in the input
    anchorPoints(:,1),yWinPos,...
    'linear','extrap'));
anaHop = [0 xWinPos(2:end)-...        % Analysis hopsizes
    xWinPos(1:end-1)];
...
xWinPos = xWinPos + tol;            % Compensate for the extra 'tol'
...
for c = 1 : numOfChan                   % loop over channels
    ...
    del = 0;                                % Shift of the current analysis
                                            % window position
        for i = 1 : length(xWinPos)-1
            % OLA
            currSynWinRan = ...                 % The range of the current
             yWinPos(i):yWinPos(i)+winLen-1;% synthesis window
         
            currAnaWinRan = ...                 % The range of the current analysis
             xWinPos(i) + del: ...            % window, shifted by the offset
             xWinPos(i) + winLen-1 + del;     % 'del'
            ...
            nextAnaWinRan = ...                 % The range where the next analysis
             xWinPos(i+1) - tol: ...          % window could be located
             xWinPos(i+1) + winLen-1 + tol;   % (including the tolerance region)
    ...
    % process last frame
    yC(yWinPos(end):yWinPos(end)+winLen-1) = ...
        yC(yWinPos(end):yWinPos(end)+winLen-1) + ...
        xC(xWinPos(i)+del:xWinPos(i)+winLen-1+del) .* w;
    ow(yWinPos(end):yWinPos(end)+winLen-1) = ...
        ow(yWinPos(end):yWinPos(end)+winLen-1) + w;
